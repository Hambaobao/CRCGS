...
def evaluate(opt):
    # load data
    images = np.load('/data0/zl/cross-modal/process_data/coco/feature.npy')
    texts = np.load('/data0/zl/cross-modal/process_data/coco/bow.npy')
    labels = np.load('/data0/zl/cross-modal/process_data/coco/label.npy')
    labels = labels[:, :-1]
    test_list = np.load('/data0/zl/cross-modal/process_data/coco/test_data_list.npy')

    # model file
    model_path = '/data0/zl/cross-modal/student_model_save/coco/model.pt'
    binary_path = '/data0/zl/cross-modal/binary_model_save/coco/binary{}_model.pt'.format(opt.code_len)
    ...
    # build student model
    stu_model = IDCM_NN(img_input_dim=4096, img_output_dim=2048, txt_input_dim=texts.shape[-1], txt_output_dim=2048, output_dim=labels.shape[-1], minus_one_dim=semantic_embedding_size).cuda()
    ...
    # record results
    save_folder = '/data0/zl/cross-modal/final_results/coco/'
